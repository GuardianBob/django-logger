...
from logger.admin import StatusLogAdmin
...
class TestDbLogger(TestCase):
  ...
  def test_log(self):
    ...
    log = self.__test_log_aux('Critical Message', self.logger.critical, logging.CRITICAL)
    self.assertEqual(self.status_log_admin.colored_msg(log),
                      '<span style="color: red;">Critcal Message</span>')
